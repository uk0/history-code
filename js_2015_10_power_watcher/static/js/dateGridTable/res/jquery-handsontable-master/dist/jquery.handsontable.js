...
(function ($, window, Handsontable) {
    ...
    Handsontable.Core = function (rootElement, userSettings) {
        ...
        legacyEventMap = {
            ...
            onCreateAutoRow: "afterCreateAutoRow",
            ...
            alter: function (action, index, amount, source, keepEmptyRows) {
                ...
                switch (action) {
                    ...
                    case "merge":
                        ...
                        var firstcell = instance.getCell(corners[0], corners[1]);
                        var endcell = instance.getCell(corners[2], corners[3]);
                        $(firstcell).attr('rowspan', corners[2] - corners[0] + 1);
                        for (var i = 0; i < corners[2] - corners[0]; i++) {
                            var hidecell = instance.getCell(corners[0] + i + 1, corners[1]);
        ...
        function getdepth(data) {
            if (typeof data.children != 'undefined') {
                for (var i in data.children) {
                    data.children[i].depth = parseInt(data.depth) + 1;
                    getdepth(data.children[i]);
                }
            } else if (parseInt(data.depth) > rowsnum) {
                rowsnum = parseInt(data.depth);
            }
        }
        function treeToth(data) {
            var nextth = [];
            var colsnum = 0;
            datastr += '<tr>';
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (children.length == 0) { instance.getSettings().leafHeaders.push(data[i]); }
                var _c = parseInt(data[i].leafs);
                var _r = rowsnum - parseInt(data[i].depth) + 1;
                var colstr = '', rowstr = '';
                if (_c > 1) { colstr = 'colspan="' + _c + '"'; }
                if (_r > 1 && children.length == 0) { rowstr = 'rowspan="' + _r + '"'; }
                if ((rowsnum - parseInt(data[i].depth) + 1))
                    datastr += '<th id="' + data[i].id + '"' + rowstr + ' ' + colstr + '>' + data[i].name + '</th>';
            ...
            datastr += '</tr>';
            if (nextth.length > 0) { treeToth(nextth); }
        }
        this.setMcolHeaders = function (data) {
            for (var i = 0; i < data.length; i++) {
                getdepth(data[i]);
            }
            datastr = '';
            treeToth(data);
            var mythead = this.$table.children('thead');
            var firsttr = mythead.find('tr:first-child');
            firsttr.before(datastr);
            $(firsttr).remove();
        }
        /**
    ...
    HandsontableTextEditorClass.prototype.refreshDimensions = function () {
        ...
        this.textareaParentStyle.top = (editTop + 30) + 'px';
        this.textareaParentStyle.left = editLeft + 'px';
        ///end prepare textarea position

        var width = $td.width() + 10, height = $td.height() + 10;
    ...
    function createContextMenu() {
        var instance = this
            ...
            , allItems = {
                "row_above": { name: "在当前行前插入", disabled: isDisabled },
                "row_below": { name: "在当前行后插入", disabled: isDisabled },
                "hsep1": "---------",
                "col_left": { name: "在当前单元格左侧插入", disabled: isDisabled },
                "col_right": { name: "在当前单元格右侧插入", disabled: isDisabled },
                "hsep2": "---------",
                "remove_row": { name: "删除当前行", disabled: isDisabled },
                "remove_col": { name: "删除当前单元格", disabled: isDisabled },
                "hsep3": "---------",
                "undo": {
                    name: "重置", disabled: function () {
                        return !instance.isUndoAvailable();
                    }
                },
                "redo": {
                    name: "返回", disabled: function () {
                        return !instance.isRedoAvailable();
                    }
                },
                "hsep4": "---------",
                "merge": { name: "合并单元格", disabled: isDisabled }
            }
            , defaultOptions = {
    ...
    WalkontableTable.prototype.adjustAvailableNodes = function () {
        ...
        for (c = 0; c < this.colgroupChildrenLength; c++) {
            if (c < displayThs) {
                if (c == 0) {
                    this.wtDom.addClass(this.COLGROUP.childNodes[c], 'rowHeaderNo');
                } else {
    ...
    WalkontableTable.prototype.getCoords = function (TD) {
        ...
        for (var i = 0, obj = TD.parentNode.childNodes; i < obj.length; i++) {
            if (TD == obj[i]) { cellIndex = i; break; }
        }
        return [
          this.rowFilter.visibleToSource(this.wtDom.prevSiblings(TD.parentNode).length),
          this.columnFilter.visibleRowHeadedColumnToSourceColumn(cellIndex)
        ];
    };
